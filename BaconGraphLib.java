...
public class BaconGraphLib {
    private static final int infSeparation = 100;
    ...
    /**
     * Calculate the separation from a root to every connected nodes, now giving unconnected nodes score of 100.
     * @param graph the total graph of the universe
     * @param tree our Bacon Tree, created with root as the center of universe
     * @param root the center of Universe
     * @return     a map that maps nodes to its separation from the root
     */

    public static <V,E> Map<V, Integer> getWeightedSeparation(Graph<V,E> graph, Graph<V,E> tree, V root){
        Map<V, Integer> distanceFromRoot = new HashMap<>();
        distanceFromRoot.put(root, 0);
        ...
        Set<V> missingVertices = missingVertices(graph, tree);
        for (V v: missingVertices) {
            distanceFromRoot.put(v, infSeparation);
        }
        return distanceFromRoot;
    }

    /**
     * Calculate the average separation from a root to all connected nodes, weighted.
     * @param graph the total graph of the universe
     * @param tree  the Bacon Tree, initialized with root as its center of universe.
     * @param root  the center of universe
     * @return      average separation from all of root's connected nodes.
     */
    public static <V,E> double averageWeightedSeparation(Graph<V,E> graph, Graph<V,E> tree, V root){
        Map<V, Integer> distanceFromRoot = getWeightedSeparation(graph, tree, root);
        int totalSep = 0;
        for (V v: distanceFromRoot.keySet()){
            totalSep += distanceFromRoot.get(v);
        }
